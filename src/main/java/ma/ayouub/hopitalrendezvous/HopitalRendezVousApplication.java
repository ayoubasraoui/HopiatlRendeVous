package ma.ayouub.hopitalrendezvous;
...
import ma.ayouub.hopitalrendezvous.Entities.*;
import ma.ayouub.hopitalrendezvous.service.IHospitalService;
...
public class HopitalRendezVousApplication   {

	public static void main(String[] args) {
		SpringApplication.run(HopitalRendezVousApplication.class, args);
	}
	...
	CommandLineRunner start (
			...
			IHospitalService hospitalService, PatientRepo patientRepository,
			MedecinRepo medecinRepository,
			RendezvousRepo rendezVousRepository , // enregistre le rendez vous j ai besoin de injecte le
			ConsultationRepo  consultationRepository // injecte l interface pour enregistre la consltation
		...
		return  args ->  {
			Stream.of("ayoub", "achraf", "imad") // pour chercher le patient par son nom
					.forEach(name->{
			...
			Stream.of("artscape","imed","ashraf")
					.forEach(name->{
